...
'''
DEBUG
1: Inicializacion driver
2: Inicializacion sensores
10: Odometria
11: Finalizacion
'''
...
class PiBot(Kibotics):
	...
	def __init__(self):
		...
		try:
			...
			cam = cfg.getProperty('Pibot.Real.Camera')
			fps = cfg.getProperty('Pibot.Real.FPS')
			w = cfg.getProperty('Pibot.Real.Width')
			h = cfg.getProperty('Pibot.Real.Height')
			d = cfg.getProperty('Pibot.Real.Dist_ruedas')
			r = cfg.getProperty('Pibot.Real.Radio_ruedas')

			if cam == "PiCam":
				if 2 in DEBUG:
					print("[PiBot] Iniciando Camara: ", cam, "...")
					print("\t[Camara] FPS: ", fps)
					print("\t[Camara] Resolucion: ("+str(w)+","+str(h)+")")
	...
	def moverServo(self, *args):
		'''
		Funcion que hace girar al servo motor a un angulo dado como parametro.
		@type args: lista
	...
	def avanzar(self, vel):
		'''
		Funcion que hace avanzar al robot en linea recta a una velocidad dada como parametro.
		@type vel: entero
		@param vel: velocidad de avance del robot (maximo 255)
		'''
		self.move(vel, 0)
	...
	def retroceder(self, vel):
		'''
		Funcin que hace retroceder al robot en linea recta a una velocidad dada como parametro.
		@type vel: entero
		@param vel: velocidad de retroceso del robot (maximo 255)
		'''
		self.move(-vel, 0)
	...
	def parar(self):
		'''
		Funcion que hace detenerse al robot.
		'''
	...
	def girarIzquierda(self, velW):
		'''
		Funcion que hace rotar al robot sobre si mismo hacia la izquierda a una velocidad dada como parametro.
		@type vel: entero
		@param vel: velocidad de giro del robot (maximo 255)
		'''
		self.move(0, velW)
	...
	def girarDerecha(self, velW):
		'''
		Funcion que hace rotar al robot sobre si mismo hacia la derecha a una velocidad dada como parametro.
		@type vel: entero
		@param vel: velocidad de giro del robot (maximo 255)
		'''
		self.move(0, -velW)
	...
	def _alcanzarposicion(self, angle, angle_aux, ErrorRange):
		...
		while(not finish):
			...
			if(tout_excedido):
				if 10 in DEBUG:
					print("[PiBot] No se pudo alcanzar el angulo deseado", angle)
	...
	def move(self, velV, velW):
		'''
		Funcion que hace avanzar y girar al robot al mismo tiempo, segun las velocidades V,W dadas como parametro.
	...
	def dameObjeto(self, lower=ORANGE_MIN, upper=ORANGE_MAX, showImageFiltered=False):
		'''
		Funcion que devuelve el centro del objeto que tiene un color verde en el rango [GREEN_MIN, GREEN_MAX] para ser detectado
		'''
		# resize the image
	...
	def dameSonarVisual (self):
		'''
		Funcion que devuelve el array de puntos [X,Y] (Z=0) correspondiente al obstaculo detectado
		'''
		...
		while (j < ANCHO_IMAGEN): # recorrido en columnas
			...
			while ((i>=0) and (esFrontera == None)): # recorrido en filas
				...
				if (pix != 0): # si no soy negro
					...
					# Calculo los demas vecinos, para ver de que color son...
					c = j - 1
					row = i
					v1 = row*ANCHO_IMAGEN+c

					if (not((c >= 0) and (c < ANCHO_IMAGEN) and
					(row >= 0) and (row < LARGO_IMAGEN))): # si no es valido ponemos sus valores a 0
						pix = 0
					else:
						pix = bnImg[row, c]
					...
					if (esFrontera == True): # si NO SOY COLOR CAMPO y alguno de los vecinitos ES color campo...
						...
						# obtenemos su backproject e interseccion con plano Z en 3D
	...
	def quienSoy(self):
		print ("Buenos dias, humano. Soy el Robot PiBot!")
...
class Odometry:
	...
	def __init__(self, d, r, gpio):
		...
		if 2 in DEBUG:
			...
			print("\t[Odometria] Radio rueda:",r)
			print("\t[Odometria] Distancia eje:",d)
	...
	def stopOdometry(self):
		...
		if 11 in DEBUG:
			print("[Odometria] Cerrando odometria...", end="")
		self.kill_event.set()
	...
	def _pulse_in(self, inp, bit):
		...
		Devuelve el ancho de pulso (en microsegundos) del estado especificado de la senal (alto o bajo)
		'''
		if(bit != 0 and bit != 1):
			return 0
		else:
